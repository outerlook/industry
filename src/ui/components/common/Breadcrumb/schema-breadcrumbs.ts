...
const forUnits = ({ unit, ...rest }: { unit: validTypes["Unit"] } & Parameters<typeof forCompanies>[0]) => {
    return [
        ...forCompanies(rest),
...
const forAssets = ({ asset, ...rest }: { asset: validTypes["Asset"] } & Parameters<typeof forUnits>[0]) => {
    return [
        ...forUnits(rest),
...
const forUsers = ({ user, ...rest }: { user: validTypes["User"] } & Parameters<typeof forUnits>[0]) => {
    return [
        ...forUnits(rest),
...
const forWorkorders = ({ workorder, ...rest }: { workorder: validTypes["Workorder"] } & Parameters<typeof forAssets>[0]) => {
    return [
        ...forAssets(rest),
